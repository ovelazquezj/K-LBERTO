...
class TimingAnalyzer:
    ...
    def fit_models(self):
        ...
        # Modelo 1: Lineal (y = slope*x + intercept)
        p_linear = Polynomial.fit(X, Y, 1)
        y_pred_linear = p_linear(X)
        r2_linear = 1 - np.sum((Y - y_pred_linear)**2) / np.sum((Y - np.mean(Y))**2)
        
        # CORRECCIÓN: coef[0] = intercept, coef[1] = slope
        coef = p_linear.convert().coef
        intercept_lin = coef[0]
        slope_lin = coef[1]
        
        print(f"\n1. MODELO LINEAL")
        print(f"   Ecuación: y = {slope_lin:.6f}*x + {intercept_lin:.4f}")
        print(f"   R² = {r2_linear:.6f}")
        print(f"   Interpretación: Cada 1000 samples agregan {slope_lin*1000:.1f} minutos")
        ...
        coef_log = p_log.convert().coef
        intercept_log = coef_log[0]
        slope_log = coef_log[1]
        
        print(f"\n2. MODELO LOGARÍTMICO")
        print(f"   Ecuación: y = {slope_log:.4f}*log(x) + {intercept_log:.4f}")
        print(f"   R² = {r2_log:.6f}")
        ...
        coef_power = p_power.convert().coef
        log_a = coef_power[0]
        b_pow = coef_power[1]
        a_pow = np.exp(log_a)
        ...
        models = {
            'Lineal': (r2_linear, lambda x: slope_lin*x + intercept_lin),
            'Logarítmico': (r2_log, lambda x: slope_log*np.log(x) + intercept_log),
    ...
    def project_times(self, model_func, timeout_minutes=240):
        ...
        print("="*80)
        print(f"Timeout configurado: {timeout_minutes} minutos ({timeout_minutes/60:.1f} horas)\n")
    ...
    def generate_recommendations(self, results, timeout_minutes=240):
        ...
        if min_margin < 0:
            ...
            print(f"   Máximo predicho: {max_pred:.1f} min ({max_pred/60:.2f} h)")
            print(f"   Timeout actual:  {timeout_minutes} min ({timeout_minutes/60:.1f} h)")
            print(f"   Recomendado:     {recommended} min ({recommended/60:.1f} h) - 15% margen")
            ...
            print(f"   Recomendado aumentar a: {recommended} min ({recommended/60:.1f} h)")
        else:
            print(f"\n✅ TIMEOUT ADECUADO")
            print(f"   Margen mínimo: {min_margin:.1f} min ({(min_margin/timeout_minutes)*100:.1f}%)")
        ...
        print("\nNotas:")
        print("  • Proyección basada en experimentos completados")
        print("  • Variabilidad real puede diferir ±10%")
        print("  • Monitorear primeros experimentos D2000+ para validar")
