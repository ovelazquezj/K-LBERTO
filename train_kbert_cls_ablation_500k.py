#!/usr/bin/env python3
"""
K-BERT Classification Ablation: 500k triplets (FULL)
...
class KBERTClassificationAblationMonitor:
    """Integrated monitoring for K-BERT classification ablation on Jetson"""

    def __init__(self, kg_triplets=500000, output_dir="./outputs/kbert_cls"):
        ...
        # CSV file paths
        self.training_csv = self.output_dir / f'training_metrics_ablation_{kg_triplets}_{timestamp}.csv'
        self.power_csv = self.output_dir / f'power_metrics_ablation_{kg_triplets}_{timestamp}.csv'
        self.characteristics_csv = self.output_dir / f'model_characteristics_ablation_{kg_triplets}_{timestamp}.csv'

        # Log files
        self.log_file = self.output_dir / f'training_ablation_{kg_triplets}_triplets_{timestamp}.log'
        self.tegrastats_log = self.monitoring_dir / f'tegrastats_ablation_{kg_triplets}_{timestamp}.log'
    ...
    def build_training_command(self, kg_path):
        """Build K-BERT training command for ablation"""
        cmd = [
            ...
            "--warmup 0.05",
            f"--kg_name {kg_path}",
            f"--output_model_path ./outputs/kbert_cls/kbert_ablation_{self.kg_triplets}_triplets.bin",
            "--seq_length 128"
        ]
        return " ".join(cmd)

    def run_training(self, kg_path):
        """Execute training with integrated monitoring"""
        print("\n" + "="*80)
        print(f"🚀 K-BERT Classification ABLATION: {self.kg_triplets} triplets (FULL)")
        print("="*80)
        print("\nConfiguration:")
        print("  Model: BETO (Spanish BERT)")
        print("  Dataset: PAWS-X Spanish (49k train, 2k dev, 2k test)")
        print("  Task: Binary Paraphrase Detection")
        print(f"  Knowledge Graph: WikidataES ({self.kg_triplets} triplets)")
        print("  Epochs: 8")
        print("  Batch size: 16")
        ...
        cmd = self.build_training_command(kg_path)
        ...
        if process.returncode == 0:
            print(f"✓ Ablation {self.kg_triplets} completed successfully")
            print(f"  Elapsed time: {elapsed/3600:.2f} hours ({int(elapsed/60)} minutes)")
            print(f"  Model saved: ./outputs/kbert_cls/kbert_ablation_{self.kg_triplets}_triplets.bin")
        else:
...
if __name__ == "__main__":
    trainer = KBERTClassificationAblationMonitor(kg_triplets=500000)
    trainer.run_training(kg_path="./brain/kgs/ablation/WikidataES_500000_triplets.spo")
