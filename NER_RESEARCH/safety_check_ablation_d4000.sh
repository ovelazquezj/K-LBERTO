#!/bin/bash
# Safety check para experimentos de ablación KG D=4000
# Uso: ./safety_check_ablation_d4000.sh [--loop]
...
print_header() {
    echo -e "${BLUE}=== ABLATION D=4000 SAFETY CHECK - $(date '+%Y-%m-%d %H:%M:%S') ===${NC}"
}
...
check_progress() {
    echo -e "${YELLOW}📊 PROGRESO DE EXPERIMENTOS (D=4000):${NC}"
    if [ -f "$PROGRESS_FILE" ]; then
        ...
        echo "   Completados: $COMPLETED/30"
        echo "   Fallidos: $FAILED"

        if [ "$COMPLETED" -gt 0 ]; then
            echo -e "${YELLOW}   Últimos 5 completados:${NC}"
            python3 -c "import json; d=json.load(open('$PROGRESS_FILE')); [print(f'     - {x}') for x in d.get('completed', [])[-5:]]" 2>/dev/null
        fi
        ...
        if [ -n "$LAST_EXP" ]; then
            ...
            if [ -n "$START_TIME" ]; then
                ...
                if [ -n "$START_SEC" ]; then
                    ...
                    echo "   Tiempo transcurrido: ${ELAPSED} min (~$(echo "$ELAPSED/60" | bc -l | xargs printf "%.1f")h)"
                fi
            fi
        fi
    else
...
check_process() {
    ...
    PID=$(pgrep -f "run_ablation_experiments_d4000.py" | head -1)
    if [ -n "$PID" ]; then
...
check_results_summary() {
    ...
    if [ -d "$RESULTS_DIR" ]; then
        COUNT=$(find "$RESULTS_DIR" -name "*_metrics.csv" 2>/dev/null | wc -l)
        echo "   Experimentos con resultados: $COUNT/30"

        if [ "$COUNT" -gt 0 ]; then
            echo -e "${YELLOW}   Últimos 5 F1 Scores:${NC}"
            for dir in $(ls -td "$RESULTS_DIR"/ABL_*/ | head -5); do
                if [ -f "${dir}"*_metrics.csv ]; then
                    EXP_NAME=$(basename "$dir")
                    METRICS_FILE=$(ls "${dir}"*_metrics.csv | head -1)
                    F1=$(tail -1 "$METRICS_FILE" 2>/dev/null | cut -d',' -f4 2>/dev/null)
                    if [ -n "$F1" ]; then
                        echo "     $EXP_NAME: F1=$F1"
                    fi
                fi
