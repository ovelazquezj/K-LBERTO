...
OUTPUT_DIR = BASE_DIR / "ablation_analysis"
...
def generate_report(df, summary, anova_results, comparisons, output_dir):
    ...
    report.append(f"**Experiments:** {len(df)}/18\n\n")
...
def main():
    ...
    if len(df) < 18 and not args.partial:
        print(f"\n⚠️  Solo {len(df)}/18. Usa --partial para análisis parcial.")
        return
